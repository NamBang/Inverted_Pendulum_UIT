...
static TM_SPI_DataSize_t TM_SPI_SetDataSize(SPI_TypeDef* SPIx, TM_SPI_DataSize_t DataSize);
static void TM_ILI9341_DrawPixel(uint16_t x, uint16_t y, uint32_t color);
static void TM_ILI9341_DrawLine(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint32_t color);
...
{
	/* Init WRX pin */
	B_GPIO_Init(ILI9341_WRX_PORT,ILI9341_WRX_PIN,RCC_AHB1Periph_GPIOD,GPIO_Mode_OUT);
	
	/* Init CS pin */
	B_GPIO_Init(ILI9341_CS_PORT, ILI9341_CS_PIN,RCC_AHB1Periph_GPIOC,GPIO_Mode_OUT);
	
	/* Init RST pin */
	B_GPIO_Init(ILI9341_RST_PORT, ILI9341_RST_PIN,RCC_AHB1Periph_GPIOD,GPIO_Mode_OUT);
...
void TM_ILI9341_INT_Fill(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1, uint16_t color) {
	...
	TM_SPI_SetDataSize(SPI5, TM_SPI_DataSize_16b);
	
	/* Send first 65535 bytes, SPI MUST BE IN 16-bit MODE */
	TM_SPI_DMA_SendHalfWord(SPI5,color, (pixels_count > 0xFFFF) ? 0xFFFF : pixels_count);
	/* Wait till done */
	while (TM_SPI_DMA_Working(SPI5));
	
	/* Check again */
	if (pixels_count > 0xFFFF) {
		/* Send remaining data */
		TM_SPI_DMA_SendHalfWord(SPI5, color, pixels_count - 0xFFFF);
		/* Wait till done */
		while (TM_SPI_DMA_Working(SPI5));
	}
	...
	TM_SPI_SetDataSize(SPI5, TM_SPI_DataSize_8b);
}
